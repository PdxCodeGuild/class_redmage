...
def rot13E(rotation):
    ...
    # Create rot13 list to hold rotated alphabet. For example: "a == n" for 13
    rot13 = []

    
    rot13.extend(letters[rotation:len(letters)] + letters[0:rotation]) 
    ...
    for i in range(len(letters)):
        encryption_dictionary[letters[i]] = rot13[i]

    return encryption_dictionary

def rot13D(rotation):

    # Create a list of all letter characters (a - z)
    letters = list(string.ascii_lowercase)

    # Create rot13 list to hold rotated alphabet. ex "a == n" for 13
    rot13_list = []

    # Use .extend to pass the values of a list into rot13. If using  rot13.append(letters[x]), it makes a list inside a list.
    rot13_list.extend(letters[rotation:len(letters)] + letters[0:rotation]) 

    # Create an encrypted dictionary, so when I ask the user what string they want to encrypt the dictionary will be ready and set. (a: n, b: o, c: p)
    decryption_dictionary = {}
    for i in range(len(letters)):
        decryption_dictionary[rot13[i]] = letters[i]
