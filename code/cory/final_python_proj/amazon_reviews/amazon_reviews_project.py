...
# Function to pull book data within the user interface
def book_choice(u_i):

    # List of titles being displayed
    t_l = [scifi_dictionaries[num]['Title'].lower()                                                         for num in range(page[0], page[1])]

    if u_i in t_l:
        
        # Uses book as key, value as index. The user will choose a book, which will return a index value.
        t_n = {scifi_dictionaries[num]['Title'].lower(): int(num) for num in range(page[0], page[1])}

        return t_n[u_i]
    
    # List of titles being displayed for all books (work around for all books, see range for titles)
    t_l_2 = [scifi_dictionaries[num]['Title'].lower()                                                         for num in range(60)]

    if u_i in t_l_2:
        t_n = {scifi_dictionaries[num]['Title'].lower(): int(num) for num in range(60)}

        return t_n[u_i]

    elif int(u_i) in range(1, 61):
        return int(u_i) - 1
...
# Using for default variable setting, book_choice linter was yelling at me
page = page_changer(1)
    ...
    while True:
        ...
        if scifi_input == 'top books':
            ...
            while True:
                ...
                loop_perm = input("\nWould you like to look at a title ('look'), change the page ('change'), or quit? > ")
                ...
                if loop_perm == 'change':
                    ...
                    u_i = input("\nWhich book would you like to look at? > ").lower()     
                    ...
                    user_continue = input("Press enter to continue, or type 'quit' to quit... ")
            ...
            while True:
                ...
                loop_permission = input("\nWould you like to look at a title ('look'), change the page ('change'), or quit? > ")

                if loop_permission == "change":
                    change_page = input("\nWhich page would you like to go to? (1 - 6) > ")
